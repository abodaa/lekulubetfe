    ...
    (async () => {
      ...
      try {
        ...
        if (initDataEarly && initDataEarly.trim() !== "") {
          console.log(
            "✅ Fresh Telegram initData detected, refreshing session...",
            {
              initDataLength: initDataEarly.length,
              source: window?.Telegram?.WebApp?.initData
                ? "WebApp"
          ...
          const isDifferentUser =
            prevUser && prevUser.id && prevUser.id !== out.user?.id;
          if (isDifferentUser) {
          ...
          console.log("New session created:", {
            sessionId: out.sessionId ? "SET" : "MISSING",
          });
      ...
      if (
        ...
        console.log(
          "⚠️ Telegram WebApp detected but initData not ready, waiting longer...",
          {
      ...
      if (!initData || initData.trim() === "") {
        ...
        if (definitelyInTelegram) {
          console.log(
            "🔄 Final attempt: Waiting 2 more seconds for initData in Telegram environment...",
          );
          await new Promise((resolve) => setTimeout(resolve, 2000));

          // Re-check initData after final wait
          const finalInitDataFromWebApp = window?.Telegram?.WebApp?.initData;
          const finalInitDataFromHash = new URLSearchParams(
            window.location.hash.substring(1),
          ).get("tgWebAppData");
          const finalInitDataFromSearch = new URLSearchParams(
            window.location.search,
          ).get("tgWebAppData");

          const finalInitData =
            (finalInitDataFromWebApp && finalInitDataFromWebApp.trim()) ||
          ...
          if (finalInitData && finalInitData.trim() !== "") {
            console.log(
              "✅ initData found after final wait, proceeding with authentication",
            );
            try {
              ...
              console.error(
                "Telegram authentication failed after final wait:",
                e,
              );
            }
          }
        ...
        console.error(
          "No Telegram initData available - this should only happen when not accessed through Telegram",
        );
      ...
      try {
        ...
        if (out && out.sessionId) {
          ...
          console.error(
            "Telegram authentication returned invalid response:",
            out,
          );
    ...
    return (
      ...
      >
        <div className="text-center w-full max-w-sm">
          ...
          <div className="text-base sm:text-lg font-semibold mb-3 text-white px-4">
            Authenticating user...
          </div>
          ...
          <div className="flex justify-center space-x-2">
            ...
            <div
              className="w-2 h-2 bg-pink-400/70 rounded-full animate-bounce"
        ...
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <div className="bg-white/10 rounded-lg p-4">
            <p className="text-white text-sm">
              ...
              <br />
              1. Open the Lekulu Bingo bot in Telegram
              <br />
