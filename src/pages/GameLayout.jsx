  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex flex-col">
      ...
      <div className="max-w-md mx-auto w-full flex flex-col h-screen">
        {/* Top Bar: Status + Stats in one row */}
        <div className="px-3 pt-2 pb-1 flex-shrink-0">
          <div className="flex items-center justify-between gap-2">
            {/* Left: Status Badge + Controls */}
        ...
        {/* Current Call - Big centered number */}
        <div className="px-3 pb-1 flex-shrink-0 flex justify-center">
          {currentNumber ? (
            <div className="bg-gradient-to-r from-yellow-500 to-orange-600 rounded-xl px-6 py-2.5 shadow-lg shadow-orange-500/40">
            ...
            <div className="bg-white/5 rounded-xl px-6 py-2.5 border border-white/10">
        ...
        {/* Number Board - Full table with rows/cols like the image */}
              ...
              <thead>
                <tr>
                  {["B", "I", "N", "G", "O"].map((letter, i) => (
                    <th
                      key={letter}
                      className={`text-center text-[11px] font-black py-1.5 ${
                        [
                          "text-blue-300 bg-blue-600/40",
                          "text-green-300 bg-green-600/40",
                          "text-purple-300 bg-purple-600/40",
                          "text-red-300 bg-red-600/40",
                          "text-yellow-300 bg-yellow-600/40",
                        ][i]
                      }`}
                    >
                      {letter}
                ...
                {Array.from({ length: 15 }, (_, rowIdx) => (
                  <tr key={rowIdx}>
                    {[0, 1, 2, 3, 4].map((colIdx) => {
                      const n = colIdx * 15 + rowIdx + 1;
                      const isCalled = calledNumbers.includes(n);
                      const isCurrent = currentNumber === n;
                      return (
                        <td
                          key={n}
                          className={`text-center text-[10px] py-0.5 font-bold border-b border-white/5 ${
                            isCurrent
                              ? "bg-orange-500 text-white font-extrabold rounded-sm shadow-lg shadow-orange-500/50"
                              : isCalled
        ...
        <main className="flex-1 px-3 pb-3 overflow-y-auto flex flex-col min-h-0">
          <div className="flex-1 flex items-center justify-center">
            {hasSingleCartela ? (
              <div className="bg-white/5 backdrop-blur rounded-2xl p-3 border border-white/10 w-full max-w-[240px] mx-auto">
